    ...
    <div class="card border border-success">
        <div class="card-header">
            <h6 class="card-title">
                ...
                Monthly Restaurant (Kitchen Orders) Revenue Report
            </h6>
        </div>
                ...
                <div class="tile">
                    <div class="embed-responsive">
                        <div class="table-responsive">
                            <table class="table table-bordered" id="monthly-restaurant-revenue-table">
                                <thead>
                                    <tr>
                                        <th>Year</th>
                                        <th>Month</th>
        ...
        <script>
            const ajaxUrl = @json(route('reports.revenue.restaurant.monthly.ajax'));
            const cat = 'monthly-restaurant-revenue';
            const token = "{{ csrf_token() }}";
            var title = "Monthly Restaurant Revenue Report";
            var table = $('#monthly-restaurant-revenue-table');
            var columns = [0, 1, 2];
            ...
            if (report != undefined || report.length > 0) {
                eBarGraph('bar_chart', 'Bargraph', report.revenue,
                    report.months, 'revenue', '#ffc107');
            }

            if (report != undefined || report.length > 0) {
                eLineGraph('line_chart', 'Linegraph', report.revenue,
                    report.months, 'revenue', '#d63384');
            }
